...
create user 'ttn_db_user'@'localhost' identified by 'ttndbuser';
create user 'ttn_app_user'@'localhost' identified by 'ttnappuser';
use ttn_affliation_body;
grant select,update,insert,create,grant option on ttn_affliation_body to 'ttn_db_user'@'localhost';
use ttn_affliation_body;
grant select,update,insert,delete on ttn_affliation_body.* to 'ttn_app_user'@'localhost';
create table if not exists Application_status( Id int(100) primary key not null, Application_status varchar(10) not null);
create table if not exists Role_ttn(RId int(100) primary key not null, Roles_assigned varchar(100) not null); 
