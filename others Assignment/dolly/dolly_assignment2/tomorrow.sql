...
create table qualification_pack(
 ...
 name varchar (209),
 e_id int(20),
 foreign key (e_id) references eligibilty(e_id));
...
INSERT INTO `eligibilty` (`e_id`, `education`) VALUES ('1', '10th ');
INSERT INTO `eligibilty` (`e_id`, `education`) VALUES ('2', '12th');
INSERT INTO `eligibilty` (`e_id`, `education`) VALUES ('3', 'graduation');
INSERT INTO `eligibilty` (`e_id`, `education`) VALUES ('4', 'post_graduation');
...
(select tc.tp_id from training_centre as tc join batches as b on tc.tc_id = b.tc_id group by b.tc_id);
 ...
 ##2
(select count(candidate.c_id),batches.q_id from candidate 
	inner join batch_candidate on candidate.c_id=batch_candidate.c_id 
    inner join batches on batches.b_id=batch_candidate.b_id group by batches.q_id) ;

(select count(candidate.c_id),batches.q_id from candidate 
	inner join batch_candidate on candidate.c_id=batch_candidate.c_id and certified=1
    inner join batches on batches.b_id=batch_candidate.b_id group by batches.q_id  ) ;

 (select count(x.c_id),batches.q_id from candidate 
inner join (select c_id from batch_candidate join batches where batches.b_id = batch_candidate.b_id and batches.a_id is not null and batches.end_date <curdate() )x
 on x.c_id=candidate.c_id
 inner join batch_candidate on candidate.c_id=batch_candidate.c_id 
inner join batches on batches.b_id=batch_candidate.b_id group by batches.q_id  ); 
...
###q1
select count(c_id),course from candidate where certified=0 group by course;


#q2
select c_id,name,age,adhaar,address,contact,email from candidate where age<25 and age>18 and certified=1 and course='waste picker';


#q3
select q_id as id ,name from qualification_pack where e_id = (select e_id from eligibilty where education='12th');
select* from eligibilty;

##q4
select * from candidate where address="rohini" and course="solar designer" and certified = 1 ;

##q5
select candidate.c_id,candidate.name from batch_candidate join candidate on batch_candidate.c_id=candidate.c_id and batch_candidate.marks is null;

##q6
select sum(batches.num_candidates),batches.tc_id,training_centre.name from batches 
join training_centre on batches.tc_id=training_centre.tc_id and batches.end_date <curdate() group by (training_centre.name);

##q7

select batch_candidate.c_id,candidate.name,candidate.course ,batch_candidate.marks from batch_candidate join candidate on batch_candidate.c_id = candidate.c_id and batch_candidate.marks>95;

##q8
select candidate.c_id,candidate.name from batch_candidate join candidate on batch_candidate.c_id=candidate.c_id and batch_candidate.marks <60 and batch_candidate.attendence>75;



##dolly q9//how many candiates are enrolled in different centres.

 select * from batches;
 select tc_id,num_candidates from batches group by tc_id;
 
 ##dollyq10//total no of batches at different centre.
 select tc_id,count(b_id) from batches group by tc_id;
 
 
 
 ####7
 select count(training_centre.tc_id),training_partner.tp_id from training_centre 
 inner join training_partner on training_partner.tp_id=training_centre.tp_id
 inner join batches on batches.tc_id=training_centre.tc_id and curdate()>batches.start_date and curdate()<batches.end_date
 group by training_partner.tp_id;
 
 
 ###6
 
 select count(b_id) from batches where year(end_date) > year(curdate())-5;
 select * from batches;
 
 ###11
 
 select training_partner.name,batches.q_id from training_partner 
 inner join training_centre on training_centre.tp_id=training_partner.tp_id
 inner join batches on batches.tc_id=training_centre.tc_id and batches.num_candidates>30 
 group by batches.q_id;
 
 ###################################vishakha###################################################3
 
 #5 who did not give exam
select candidate.c_id , candidate.name from batch_candidate join candidate on batch_candidate.c_id = candidate.c_id and  batch_candidate.marks is null;

#select sum(q.num_candidates) from (batches join (select t.tc_id from training_centre ) as t) as q where q.tc_id =.tc_id and  asses_date < curdate() group by (q.tc_id);


##V7

select batch_candidate.c_id,  candidate.name	 from batch_candidate join candidate on batch_candidate.c_id= candidate.c_id and batch_candidate.marks>95;


###1 faileds
select * from batch_candidate;
select count(c_id) from batch_candidate where attendence >75 and marks < 60;



#3
select * from qualification_pack;
select count(q_id) , name from qualification_pack where eligibility= '10' group by (name);
##2
#4
select * from batches;
# 6 
select sum( batches.num_candidates ), batches.tc_id, training_centre.name from batches join training_centre  on batches.tc_id=training_centre.tc_id and batches.end_date<curdate() group by (training_centre.name);
